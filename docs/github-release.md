...
0.5.2 起，安装器按 SHA256 比较网络驱动，内容相同则保留，避免普通应用升级要求重启电脑；仅在确实需要替换被占用的驱动时保留重启提示。
...
2. 提交并推送代码，再推送同名标签，例如版本号 `0.5.2` 对应 `v0.5.2`。
...
开发者本地也可以执行 `tools/build_release.ps1`，需要 Python 3.12、requirements.txt 中依赖，以及 Inno Setup 6.4 或更新版本。安装包和更新清单输出到 `deliverables`；构建目录、本机日志、环境和安装包不提交到 Git。

`python tools/test_installer_driver.py` 使用正式安装脚本中的驱动判断逻辑，验证缺失、内容变化、内容相同和相同文件被占用四种情况。测试仅使用隔离文件，不安装或加载真实驱动。
